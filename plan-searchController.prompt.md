...
2. Code changes — Controller:
   ...
   - **Добави нов** `SearchController` (файл: Controllers/SearchController.cs) с действия:
     ...
     - `SaveOperatorEdits()` (POST): приема список от (RowIndex, NewEik) от формата, валидира формат на ЕИК (9 или 12 цифри или нищо), зарежда оригиналния XLSX, заменя `!!!!` на попълнените стойности само за редовете, за които операторът е въвел валиден ЕИК, и записва нов файл в `Downloads` с името `<original>-operator-result.xlsx`. Възвраща URL/име на новия файл или JSON резултат.
...
4. Save/Edit logic:
   ...
   - Замяната трябва да се прави само за редовете със стара стойност `!!!!` и само ако операторът е въвел валиден ЕИК (9 или 12 цифри или нищо). Невалидните входове се игнорират и връщат като грешки/съобщения за потребителя.
...
- Попълваме само `!!!!` стойности и само ако операторът въведе валиден 9 или 12-цифрен или нищо ЕИК.
